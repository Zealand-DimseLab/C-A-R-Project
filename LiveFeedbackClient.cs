    ...
    public class LiveFeedbackClient : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            while (_incoming.TryDequeue(out string msg))
            {
                ...
                {
                    // Log at Debug level so it doesn't pollute the console during normal play.
                    // The raw message is included to help diagnose unexpected server responses.
                    Debug.Log($"[LiveAnalytics] Feedback parse skipped ({ex.Message}): {msg}");
                }
            }
#endif
        }
        ...
        {
#if !(UNITY_WEBGL && !UNITY_EDITOR)
            // 1. Trigger token cancellation so loops know to break if they can
            if (_cts != null)
            {
                _cts.Cancel();
                _cts.Dispose();
            ...
            // 2. Safe, crash-proof native socket termination
            if (_ws != null)
            {
                try
                {
                    // CRITICAL: Abort() forces the underlying native C++ socket handles 
                    // to close cleanly, canceling active async I/O transfers safely.
                    _ws.Abort(); 
                    _ws.Dispose();
                }
                catch (Exception) 
                {
                    // Quietly catch native disposal anomalies during teardown
                }
                finally
                {
                    _ws = null;
