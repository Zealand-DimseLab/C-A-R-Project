...
public class WPVehicleController_BAK : MonoBehaviour {
    public enum WaypointSpeed { Cruise, Slow }

    [System.Serializable]
    public class WaypointBehaviour_BAK {
    ...
    public List<WaypointBehaviour_BAK> waypointBehaviours = new List<WaypointBehaviour_BAK>();
    ...
    void SyncBehaviourList() {
        while (_path != null && waypointBehaviours.Count < _path.Length)
            waypointBehaviours.Add(new WaypointBehaviour_BAK());
    }
    ...
    void Update() {
        ...
        WaypointBehaviour_BAK behaviour = waypointBehaviours[_i];
    ...
    bool TryGetActiveStopTarget(out int stopIndex, out StopReason reason, out RoadSignalDurationController signal) {
        ...
        for (int idx = start; idx <= end; idx++) {
            WaypointBehaviour_BAK b = waypointBehaviours[idx];
