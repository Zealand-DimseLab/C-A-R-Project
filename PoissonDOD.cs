...
public class PoissonDOD{
    ...
    public GrassDataAssetDOD bakedData;

    public GrassDataAssetDOD BakeGrass() {
#if UNITY_EDITOR
        ...
        float[] yaws = new float[points.Count];
        float[] scales = new float[points.Count];

        for(int i = 0; i < points.Count; i++){
            Vector3 rayStart = new Vector3(
                    this.transform.position.x - this.regionSize.x * 0.5f + points[i].x,
                    this.transform.position.y + 500f,
                    this.transform.position.z - this.regionSize.y * 0.5f + points[i].y
                    );
