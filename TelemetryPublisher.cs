...
namespace LiveAnalytics
{
    ...
    public class TelemetryPublisher : MonoBehaviour {
        [SerializeField] private TMPro.TMP_Text currentSessionIDText;
        ...
        [Header("Auto-wired sources (assign or leave blank for auto-find)")]
        [Tooltip("Wahoo BLE WebSocket client — auto-found if not assigned.")]
        [SerializeField] private WahooWsClient _wahooWsClient;

        [Tooltip("Arduino serial reader for speed — auto-found if not assigned.")]
        [SerializeField] private ArduinoSerialReader _arduinoSerialReader;
        ...
        void Start()
        {
            ...
            currentSessionIDText.text = _sessionId;
            ...
            // Auto-find WahooWsClient if not assigned in Inspector.
            if (_wahooWsClient == null)
                _wahooWsClient = FindAnyObjectByType<WahooWsClient>();
                // _wahooWsClient = FindObjectOfType<WahooWsClient>();
            if (_wahooWsClient != null)
            {
                _hrHandler = hr => externalHeartRate = hr;
                _wahooWsClient.OnHeartRate += _hrHandler;
            }
            else
                Debug.LogWarning("[LiveAnalytics] WahooWsClient not found — heart_rate will be 0.");

            // Auto-find ArduinoSerialReader if not assigned in Inspector.
            if (_arduinoSerialReader == null)
                _arduinoSerialReader = FindAnyObjectByType<ArduinoSerialReader>();
                // _arduinoSerialReader = FindObjectOfType<ArduinoSerialReader>();
        ...
        void OnDestroy() {
            ...
            // if (_wahooWsClient != null && _hrHandler != null)
            //     _wahooWsClient.OnHeartRate -= _hrHandler;
        }
        ...
        private void SafeUnsubscribe() {
            // If the Wahoo client still exists, drop our handle cleanly
            if (_wahooWsClient != null && _hrHandler != null) {
                _wahooWsClient.OnHeartRate -= _hrHandler;
            }
            _hrHandler = null;
        ...
        private void CleanupTelemetry() {
            // Prevent double flushing if called by both OnApplicationQuit and OnDestroy
            ...
            try {
                ...
                System.Console.WriteLine($"[LiveAnalytics] Teardown exception caught: {ex.Message}");
            }
            finally {
                _wsClient = null;
            }
        ...
        private void EnqueueGameplay()
        {
            ...
            r.speed = _arduinoSerialReader != null ? _arduinoSerialReader.speed : externalSpeed;
